...
export const SignInUpContextProvider = (props) => {
  ...
  const getUsers = async () => {
    ...
    const filteredProfiles = profiles.docs.map((user) => ({
      ...user.data(),
      profileId: user.id,
    }));
    setFilterProfiles(filteredProfiles);
    filteredProfiles.map((profileId) => {
      if (profileId.userId === currentUserId) {
        setCurrentUserDocId(profileId.profileId);
      }
    });
  };
  const addChatRoom = (a) => {
  ...
  onSnapshot(currentUserChats, (snapshot) => {
    ...
    if (profileDataArray.length !== 0) {
      ...
      const nededChat = chats.find(
        (chat) => chat.TalkingTo === profileTalkingTo.profileId,
      );
      if (
        currentMessages != null &&
        currentMessages.length != nededChat.Messages.length
      ) {
  ...
  const getCurrentUserImage = async (a, b) => {
    if (a === currentUserId) {
      ...
      await getDownloadURL(profileImg).then((url) => {
        setUserImg(url);
      });
    } else if (a != undefined) {
      const profileImg = ref(storage, `/profileImages/${a}`);
  ...
  const changeProfileTalkingTo = (a, b, c) => {
    setProfileTalkingTo({ profileName: a, profileImage: b, profileId: c });
  };
  ...
  return (
    <SignInUpContext.Provider value={contextValue}>
      {props.children}
